...
export function normalizeFoodicsOrder(payload: Record<string, any>): NormalizedSale {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const lineItems = (order.order_products ?? order.products ?? order.items ?? []).map((item: any) => ({
    description: item.product?.name ?? item.name ?? "Item",
    quantity: Number(item.quantity ?? 1),
    unitPrice: Number(item.unit_price ?? item.price ?? 0),
    total: Number(item.total_price ?? item.total ?? 0),
  }));

  const pm = (order.payment_method ?? order.tender_type ?? "").toLowerCase();
  const paymentMethod =
  ...
  return {
    ...
    occurredAt: new Date(order.opened_at ?? order.created_at ?? Date.now()),
    subtotal,
    vatAmount,
    total,
    currency,
